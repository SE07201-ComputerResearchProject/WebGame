...
const FriendsSidebar = ({ isOpen, onClose, onOpenChat }: FriendsSidebarProps) => {
  ...
  return (
    <>
      {isOpen && (
        <div className="fixed inset-0 bg-background/50 backdrop-blur-sm z-40 lg:hidden" onClick={onClose} />
      )}

      <aside className={`fixed top-0 right-0 h-full w-80 glass-card border-l border-border/50 z-50 transform transition-transform duration-300 ${isOpen ? "translate-x-0" : "translate-x-full"}`}>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-gaming px-4 pb-4">
          ...
          {offlineFriends.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">Ngoại tuyến — {offlineFriends.length}</p>
              <div className="space-y-1">
...
const FriendItem = ({ friend, onChat }: { friend: Friend; onChat: () => void }) => {
  const statusColor = { online: "text-neon-green", playing: "text-neon-cyan", offline: "text-muted-foreground" };
  return (
    <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted/30 transition-colors group cursor-pointer">
      <div className="relative">
        <img src={friend.avatar} alt={friend.name} className={`w-10 h-10 rounded-full object-cover border-2 ${friend.status === "offline" ? "border-muted opacity-60" : "border-primary/50"}`} />
