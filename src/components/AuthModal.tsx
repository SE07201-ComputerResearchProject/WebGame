...
const AuthModal = ({ isOpen, onClose }: AuthModalProps) => {
  ...
  const processLoginSuccess = (res: any) => {
    if (res?.mfaRequired) {
      ...
      toast({ title: "Lỗi", description: res?.error || "Đăng nhập thất bại", variant: "destructive" }); 
  ...
  const validateForm = () => {
    ...
    if (mode === "register") {
      ...
      if (formData.username.length < 3 || formData.username.length > 20) {
        newErrors.username = "Tên đăng nhập phải từ 3 đến 20 ký tự.";
        isValid = false;
      } else if (!/^[a-zA-Z0-9_]+$/.test(formData.username)) {
        newErrors.username = "Chỉ được dùng chữ cái, số và dấu gạch dưới (_).";
        isValid = false;
      }
    ...
    // Check Email (Chung cho cả Login và Register)
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email)) {
      newErrors.email = "Định dạng email không hợp lệ.";
      isValid = false;
    }
    // Check Password (Chung)
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!captchaToken) return toast({ title: "Cảnh báo", description: "Vui lòng xác nhận bạn không phải Robot!", variant: "destructive" });
         ...
         {mfaStep ? (
           ...
           <div className="animate-fade-in">
             ...
             <form onSubmit={handleSubmit} className="space-y-4">
               {mode === "register" && (
                 <div>
                   <div className="relative"><User className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" /><input type="text" placeholder="Tên người dùng (3-20 ký tự)" value={formData.username} onChange={(e) => { setFormData({ ...formData, username: e.target.value }); setErrors({...errors, username: ""}) }} className={`w-full h-12 pl-12 pr-4 rounded-xl bg-muted/50 border ${errors.username ? 'border-destructive focus:ring-destructive/50' : 'border-border/50 focus:ring-primary/50'} text-foreground focus:ring-2 transition-all`} required /></div>
                   {errors.username && <p className="text-destructive text-xs mt-1 ml-2">{errors.username}</p>}
                 </div>
               )}
               
               <div>
                 <div className="relative"><Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" /><input type="email" placeholder="Email" value={formData.email} onChange={(e) => { setFormData({ ...formData, email: e.target.value }); setErrors({...errors, email: ""}) }} className={`w-full h-12 pl-12 pr-4 rounded-xl bg-muted/50 border ${errors.email ? 'border-destructive focus:ring-destructive/50' : 'border-border/50 focus:ring-primary/50'} focus:ring-2 transition-all`} required /></div>
                 {errors.email && <p className="text-destructive text-xs mt-1 ml-2">{errors.email}</p>}
               </div>

               <div>
                 <div className="relative">
                   <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" /><input type={showPassword ? "text" : "password"} placeholder="Mật khẩu (Ít nhất 6 ký tự)" value={formData.password} onChange={(e) => { setFormData({ ...formData, password: e.target.value }); setErrors({...errors, password: ""}) }} className={`w-full h-12 pl-12 pr-12 rounded-xl bg-muted/50 border ${errors.password ? 'border-destructive focus:ring-destructive/50' : 'border-border/50 focus:ring-primary/50'} focus:ring-2 transition-all`} required />
                 ...
                 {errors.password && <p className="text-destructive text-xs mt-1 ml-2">{errors.password}</p>}
               </div>

               {mode === "register" && (
                 <div>
                   <div className="relative"><Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" /><input type={showPassword ? "text" : "password"} placeholder="Xác nhận mật khẩu" value={formData.confirmPassword} onChange={(e) => { setFormData({ ...formData, confirmPassword: e.target.value }); setErrors({...errors, confirmPassword: ""}) }} className={`w-full h-12 pl-12 pr-4 rounded-xl bg-muted/50 border ${errors.confirmPassword ? 'border-destructive focus:ring-destructive/50' : 'border-border/50 focus:ring-primary/50'} text-foreground focus:ring-2 transition-all`} required /></div>
                   {errors.confirmPassword && <p className="text-destructive text-xs mt-1 ml-2">{errors.confirmPassword}</p>}
                 </div>
               )}
             ...
             <p className="text-center text-sm text-muted-foreground mt-6">{mode === "login" ? "Chưa có tài khoản?" : "Đã có tài khoản?"} <button type="button" onClick={() => { setMode(mode === "login" ? "register" : "login"); setCaptchaToken(null); setErrors({username:"", email:"", password:"", confirmPassword:""}); setFormData({username:"", email:"", password:"", confirmPassword:""}) }} className="text-primary hover:underline font-medium">{mode === "login" ? "Đăng ký ngay" : "Đăng nhập"}</button></p>
           </div>
         )}
       </div>
      </div>
