...
const MfaSetupModal = ({ isOpen, onClose }: MfaSetupModalProps) => {
  ...
  // SỬ DỤNG CREATE PORTAL ĐỂ RÚT MODAL RA KHỎI NAVBAR
  const modalContent = (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-card/95 backdrop-blur-md border border-border/50 shadow-2xl rounded-2xl overflow-hidden animate-slide-up p-8 z-10">
        <button onClick={onClose} className="absolute top-4 right-4 p-2 rounded-lg text-muted-foreground hover:bg-muted/50 hover:text-foreground transition-colors"><X className="w-5 h-5" /></button>
        <div id="recaptcha-setup"></div>
        ...
        {isLoading ? (
          ...
          <div className="text-center animate-fade-in">
            ...
            <form onSubmit={handleDisable} className="space-y-4">
              <input type="text" maxLength={6} placeholder="Nhập 6 số..." value={verifyCode} onChange={(e) => setVerifyCode(e.target.value.replace(/\D/g, ''))} className="w-full h-12 text-center text-xl tracking-widest rounded-xl bg-muted/50 border border-destructive/30 text-destructive focus:outline-none focus:ring-2 focus:ring-destructive/50" />
          ...
          <div className="text-center animate-fade-in">
            ...
            <div className="space-y-3">
              <button onClick={handleSelectApp} className="w-full flex items-center gap-4 p-4 rounded-xl border border-border/50 hover:border-primary/50 text-left transition-colors"><QrCode className="w-6 h-6 text-primary" /><div><p className="font-bold">App xác thực</p></div></button>
              <button onClick={() => setSetupMode("sms")} className="w-full flex items-center gap-4 p-4 rounded-xl border border-border/50 hover:border-primary/50 text-left transition-colors"><MessageSquare className="w-6 h-6 text-neon-green" /><div><p className="font-bold">Tin nhắn SMS</p></div></button>
            </div>
          </div>
        ) : setupMode === "sms" ? (
          <div className="text-center animate-fade-in">
            <MessageSquare className="w-16 h-16 text-neon-green mx-auto mb-4" /><h2 className="text-2xl font-bold mb-2">Xác thực SMS</h2>
            {smsStep === "phone" ? (
              <form onSubmit={handleSendPhone} className="space-y-4 mt-6">
                <input type="text" placeholder="Số điện thoại..." value={phone} onChange={(e) => setPhone(e.target.value.replace(/\D/g, ''))} className="w-full h-12 px-4 rounded-xl bg-muted/50 border border-border/50 focus:outline-none focus:ring-2 focus:ring-primary/50" />
              ...
              <form onSubmit={handleEnable} className="space-y-4 mt-6">
                <input type="text" maxLength={6} placeholder="000000" value={verifyCode} onChange={(e) => setVerifyCode(e.target.value.replace(/\D/g, ''))} className="w-full h-12 text-center text-xl tracking-widest rounded-xl bg-muted/50 border border-border/50 focus:outline-none focus:ring-2 focus:ring-primary/50" />
                <Button variant="gaming" className="w-full" type="submit" disabled={verifyCode.length !== 6}>Xác nhận</Button>
                <button type="button" onClick={() => setSmsStep("phone")} className="text-xs mt-2 hover:underline">Đổi số</button>
              </form>
            )}
          </div>
        ) : (
          <div className="text-center animate-fade-in">
            ...
            {qrCodeUrl && <img src={qrCodeUrl} className="w-40 h-40 mx-auto my-4 bg-white p-2 rounded-xl shadow-inner" />}
            <form onSubmit={handleEnable} className="space-y-4">
              <input type="text" maxLength={6} placeholder="Nhập 6 số..." value={verifyCode} onChange={(e) => setVerifyCode(e.target.value.replace(/\D/g, ''))} className="w-full h-12 text-center text-xl tracking-widest rounded-xl bg-muted/50 border border-border/50 focus:outline-none focus:ring-2 focus:ring-primary/50" />
